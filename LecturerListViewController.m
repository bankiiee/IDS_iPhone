...
-(void)reloadFeed{
    ...
    NSURL *url = [NSURL URLWithString:@"http://localhost:8084/IDS/lecturerFeed.xml"];
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    if(isReadingTitle == YES){
        ...
        [self.feedElement setObject:string forKey:@"name"];
    }
    if(isReadingDescription == YES){
        NSLog(@"Found DESCRIPTION %@", string);
        [self.feedElement setObject:string forKey:@"room"];
    }
    if(isReadingLink == YES){
        NSLog(@"Found ATTCH %@", string);
        [self.feedElement setObject:string forKey:@"status"];
        
    }
    if(isReadingNewsID == YES){
        NSLog(@"Found NewsID %@", string);
        [self.feedElement setObject:string forKey:@"id"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Configure the cell...
    NSString *name = [NSString stringWithFormat:@"%@",[[self.feed objectAtIndex:indexPath.row]objectForKey:@"name"]];
    NSString *room = [NSString stringWithFormat:@"%@",[[self.feed objectAtIndex:indexPath.row]objectForKey:@"room"]];
    NSString *status = [NSString stringWithFormat:@"%@",[[self.feed objectAtIndex:indexPath.row]objectForKey:@"status"]];
    [cell.textLabel setText:name];
    //[cell.detailTextLabel setText:link];
    [cell.detailTextLabel setText:[NSString stringWithFormat:@"Status : %@",status]];
    [cell.imageView setImage:[UIImage imageNamed:@"111-user.png"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    /*
     <#LecturerLecturerDetailViewController#> *detailViewController = [[<#DetailViewController#> alloc] initWithNibName:@"<#Nib name#>" bundle:nil];
     // ...
     // Pass the selected object to the new view controller.
     [self.navigationController pushViewController:detailViewController animated:YES];
     */
    NSLog(@"%@",[[self.feed objectAtIndex:indexPath.row]objectForKey:@"description"]);
    LecturerDetailViewController *lecturerViewDetail = [[LecturerDetailViewController alloc]init];
   lecturerViewDetail.name = [[self.feed objectAtIndex:indexPath.row]objectForKey:@"name"];
    lecturerViewDetail.room = [[self.feed objectAtIndex:indexPath.row]objectForKey:@"room"];
    lecturerViewDetail.status = [[self.feed objectAtIndex:indexPath.row]objectForKey:@"status"];
    lecturerViewDetail.lid = [[self.feed objectAtIndex:indexPath.row]objectForKey:@"id"];
//    
    [self.navigationController pushViewController:lecturerViewDetail animated:YES];
